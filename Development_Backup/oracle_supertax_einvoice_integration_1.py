...
def run_integration(invoice_number: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for trx_id, invoice_rows in grouped.items():
        try:
            ...
            # SuperTax expects the array wrapped in an "invoices" key
            ...
            # Write the success log to the folder
            ...
            # Write the failure log to the folder (capturing whatever payload was built before failure)
            save_payload_log(
                trx_id, 
                full_request_payload if 'full_request_payload' in locals() else None, 
                error=str(exc)
            )            
        time.sleep(1)  # gentle pacing between calls, mirrors the existing .NET job
    return results
