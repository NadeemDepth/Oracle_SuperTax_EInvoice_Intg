...
REPORT_TO_SELLERDTLS = {
    ...
    "Pin": lambda r: r.get("SELLER_PINCODE"),
    "Stcd": lambda r: r.get("SELLER_STATECODE"),
}
...
REPORT_TO_ITEM = {
    ...
    "AssAmt": lambda r: to_number(r.get("ASSESSABLE_AMOUNT")),
    "GstRt": lambda r: to_number(r.get("GST_RATE")),
...
def write_back_to_fusion(request_payload: Dict[str, Any], supertax_response: Dict[str, Any], fusion_cfg: FusionConfig, trx_id: str) -> None:
    ...
    # Standard Fusion REST endpoint for custom objects. 
    # Verify if 'Inspira_Einvoice_c' matches your exact object API name.
    ...
    # Check if SuperTax returned a success indicator (adjust "status" / "act" based on actual SuperTax response schema)
    is_success = supertax_response.get("status", "").lower() in ("act", "active", "success") or supertax_response.get("Success") == "true"
    
    # Map the payload to Fusion Custom Object fields (ensure all keys end with _c if they are custom fields)
    fusion_payload = {
        ...
        # Data from SuperTax Response (Adjust these .get() keys to match SuperTax's exact response JSON)
        "Ack_Number_c": str(supertax_response.get("AckNo", "")),
        "Ack_Date_c": supertax_response.get("AckDt", ""),
        "Record_c": supertax_response.get("Record", ""),
        "Success_c": supertax_response.get("Success", ""),        
        "FinYear_c": supertax_response.get("Fy", ""),
        "Messages_c": supertax_response.get("Messages", ""),
        "Status_c": supertax_response.get("Status", ""),
        "QR_Code_c": supertax_response.get("SignedQRCode", ""),
        "IRN_Number_c": supertax_response.get("Irn", ""),
        "RawQRCode_c": supertax_response.get("SignedQRCode", ""),
    ...
    logger.info(f"Writing record to Inspira_Einvoice for Trx_Id: {trx_id}...")
