...
def run_integration(target_trx_id: str = None) -> List[Dict[str, Any]]:
    ...
    if target_trx_id:
        if target_trx_id in grouped:
            grouped = {target_trx_id: grouped[target_trx_id]}
            logger.info(f"Manual Override: Processing single Transaction ID: {target_trx_id}")
        else:
            logger.warning(f"Manual Override Failed: Transaction ID {target_trx_id} not found in pending report.")
            return [{"invoice": target_trx_id, "success": False, "error": "Not found in Oracle pending report."}]
    else:
...
def main() -> None:
    ...
    parser.add_argument("--trx-id", required=False, help="Process a single specific Transaction ID", default=None)
    args = parser.parse_args()
    
    logger.info("Starting execution for Oracle to SuperTax integration...")
    results = run_integration(target_trx_id=args.trx_id)
